...
public abstract class AbstractRoutingRedisTemplate<K, V> extends RedisTemplate<K, V> {
    ...
    /**
     * <database,index,RedisTemplate>
     */
    private volatile Table<Integer, Integer, RedisTemplate<K, V>> redisTemplates = Tables.synchronizedTable(HashBasedTable.create());
    ...
    protected RedisTemplate<K, V> determineTargetRedisTemplate() {
        Pair<Integer, Integer> pair = determineCurrentLookupKey();
        RedisTemplate<K, V> template = this.redisTemplates.get(pair.getLeft(), pair.getRight());
        if (template == null) {
            template = this.createRedisTemplateOnMissing(pair);
            this.redisTemplates.put(pair.getLeft(), pair.getRight(), template);
        }
        return template;
    }

    protected abstract RedisTemplate<K, V> createRedisTemplateOnMissing(Pair<Integer, Integer> pair);

    protected abstract Pair<Integer, Integer> determineCurrentLookupKey();
}
